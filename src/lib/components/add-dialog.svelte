<script lang="ts">
	...
	function add(result: TSearchResult) {
		...
		if ($appsStore.find((app) => app.id === result.item.appid)) {
			appsStore.update((apps) => apps.filter((app) => app.id !== result.item.appid));
		} else {
...
<Command.Dialog bind:open shouldFilter={false}>
	...
	<Command.List>
		{#if loading}
			<Command.Loading>
				<div class="flex items-center justify-center gap-2 px-4 py-6 text-sm">
			...
			<Command.Item onSelect={() => add(result)}>
				<div class="flex h-16 w-full justify-between">
					<div class="flex gap-4">
						...
						{:catch __}
							<div
								class="flex h-[64px] w-[136.93px] items-center justify-center rounded-sm bg-zinc-400"
							>
						...
						<div class="flex flex-col justify-center">
							...
							<span class="text-xs text-muted-foreground">App ID: {result.item.appid}</span>
						</div>
					</div>
					{#if $appsStore.find((app) => app.id === result.item.appid)}
